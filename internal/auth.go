...
func MakeCookie(r *http.Request, email string, name string, id string) *http.Cookie {
	expires := cookieExpiry()
	mac := cookieSignature(r, email, name, id, fmt.Sprintf("%d", expires.Unix()))
	value := fmt.Sprintf("%s|%d|%s|%s|%s", mac, expires.Unix(), email, name, id)
...
// Create cookie hmac
func cookieSignature(r *http.Request, email, name, id, expires string) string {
